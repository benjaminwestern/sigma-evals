...
func TestRunnerCanScoreToolCallOnlyOutput(t *testing.T) {
	...
	client := &scriptedClient{responses: []sigma.AssistantMessage{toolCallMessage()}}
...
func TestSigmaTargetCompleterCanScoreToolCallOnlyOutput(t *testing.T) {
	t.Parallel()

	client := &scriptedClient{responses: []sigma.AssistantMessage{toolCallMessage()}}
	result, err := sigmaevals.NewTargetRunner(sigmaevals.NewSigmaTargetCompleter(client)).Run(context.Background(), sigmaevals.TargetRunSpec{
		Suite: sigmaevals.Suite{Name: "tools", Cases: []sigmaevals.Case{
			{
				ID:    "read-config",
				Input: "Read config/app.toml.",
		...
		Targets: []sigmaevals.Target{{Provider: "test", ModelID: "model"}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func toolCallMessage() sigma.AssistantMessage {
	return sigma.AssistantMessage{Content: []sigma.ContentBlock{
